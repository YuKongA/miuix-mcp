...
export async function fetchComponents(): Promise<string[]> {
  ...
  return cached<string[]>(`components@${pageUrl}`, 2 * 60 * 60 * 1000, async () => {
...
export async function fetchComponentDemo(componentName: string): Promise<string> {
  ...
  return cached<string>(`componentDemo@${componentName}`, 12 * 60 * 60 * 1000, async () => {
    const response = await fetch(url);
    if (!response.ok) {
...
export async function fetchComponentDoc(componentName: string): Promise<string> {
  ...
  return cached<string>(`componentDoc@${componentName.toLowerCase()}`, 12 * 60 * 60 * 1000, async () => {
    const response = await fetch(url);
    if (!response.ok) {
      if (response.status === 404) {
...
export async function fetchExampleFile(filePath: string): Promise<string> {
  ...
  return cached<string>(`exampleFile@${filePath}`, 24 * 60 * 60 * 1000, async () => {
    const response = await fetch(url);
    if (!response.ok) {
      if (response.status === 404) {
